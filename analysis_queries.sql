...
select item_purchased,round (avg(review_rating),2) as 'Average_Product_Rating' from customer group by item_purchased order by avg(review_rating) desc limit 5;
...
select subscription_status,count(*) from customer where previous_purchases > 5 group by subscription_status;

Q10. What is the revenue contribution of each age group?
select age_group,sum(purchase_amount) as total_revenue
from customer group by age_group order by total_revenue;
